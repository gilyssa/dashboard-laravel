            ...
            <div class="card mb-4 mx-4">
                <div class="card-header pb-0">
                    <div class="d-flex flex-row justify-content-between">
                        ...
                        @if ($userAccess === 'admin') 
                        <a href="{{ url('/city-management-register')}}" class="btn bg-gradient-primary btn-sm mb-0 d-block d-sm-inline-block text-icon" type="button">
                            <span class="icon">
                              <i class="fas fa-plus text-white"></i>
                            </span>
                            <span class="text d-none d-sm-inline" style="width: 50%; height: 50%;">Cadastrar Cidade</span>
                          </a>
                          <a href="{{ url('/city-management-removed')}}" class="btn bg-gradient-primary btn-sm mb-0 text-icon" type="button">
                            <span class="icon">
                              <i class="fas fa-trash text-white"></i>
                            </span>
                            <span class="text d-none d-sm-inline" style="width: 50%; height: 30%;">Cidades Removidas</span>
                          </a>             
                        @endif
                    </div>
                </div>
                            ...
                            <tbody>
                                @foreach ($cities as $city)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <p class="text-xs font-weight-bold mb-0">{{ $city['status'] ? 'Ativo' : 'Inativo' }}</p>
                                        </td>
                                        <td class="text-center">
                                        ...
                                        @if ($userAccess === 'admin') 
                                        <td class="text-center">
                                            <a class="mx-3" data-bs-toggle="tooltip" data-bs-original-title="Edit city">
                                                <i class="cursor-pointer fas fa-building text-secondary" onclick="updateCity({{ $city['id'] }});"></i>
                                            </a>
                                            <span>
                                                <i class="cursor-pointer fas fa-trash text-secondary" onclick="deleteCity({{ $city['id'] }});" ></i>
                                            </span>
                                        </td>
                                        @endif
                                    </tr>
